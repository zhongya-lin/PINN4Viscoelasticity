...
def pre_process(args):
    ...
    xy_cr = np.sqrt(xy[:, 0]**2 + xy[:, 1]**2)
    xy_ct_0 = np.arctan2(xy[:, 1], xy[:, 0])
    xy_ct = np.where(xy_ct_0 > np.pi/2.0, np.pi-xy_ct_0, xy_ct_0)
    xy_c = np.column_stack((xy_cr, xy_ct))

    xy_out_cr = np.sqrt(xy_out[:, 0]**2 + xy_out[:, 1]**2)
    xy_out_ct_0 = np.arctan2(xy_out[:, 1], xy_out[:, 0])
    # xy_out_ct = xy_out_ct_0
    xy_out_ct = np.where(xy_out_ct_0 > np.pi/2.0, np.pi-xy_out_ct_0, xy_out_ct_0)
    xy_out_c = np.column_stack((xy_out_cr, xy_out_ct))

    x_train = [torch.from_numpy(xy_c), torch.from_numpy(xy_out_c)]

    # # # Define the displacement boundary conditions
    pressure_out_r = torch.tensor(-1.0e-2)

    y_train = [pressure_out_r, torch.from_numpy(wt)]
