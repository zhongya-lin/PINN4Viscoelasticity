...
class Adam:
    def __init__(self, pinn, x_train, y_train, dt, delta_t, initial_growth, learning_rate=5e-4, maxiter=2000, visco=False, net_name='model_name'):
        ...
        self.growth_ratio = initial_growth
        self.growth_ratio_list = []
        self.dt = dt
        self.delta_t = delta_t
        self.each_iter = 1000

    def pi_loss(self, weights, growth_ratio):

        self.set_weights(weights)

        loss, grads, l1, l2 = self.loss_grad(self.x_train, self.y_train, growth_ratio)
    ...
    def loss_grad(self, x, y, growth_ratio):
        y_p = self.pinn(x, self.dt, self.delta_t, growth_ratio)

        loss, l1, l2, growth_ratio = energy_loss(y_p, y)
        self.growth_ratio = torch.from_numpy(growth_ratio.detach().numpy())
    ...
    def fit(self):
        ...
        v = np.zeros_like(x)
        b_w = 0
        loss, g = self.pi_loss(x, self.growth_ratio)
        for i in range(0, self.maxiter):
            ...
            if self.visco_bool:
                if i % self.each_iter == 0:
                    self.dt = int(i / self.each_iter) * 1.0 + 1.0

            if (i+1) % self.each_iter == 0:
                print('The time is ', self.dt, '. And the growth ratio is ', self.growth_ratio)
                self.growth_ratio_list.append(self.growth_ratio)
                np.savetxt("GrowthRatio.csv", np.array(self.growth_ratio_list))
            ...
            # learning rate decay
            # if i < 4000:
            #     learning_rate = 1e-4 / (1 - beta1**(int(i/400)+1))
            # else:
            #     learning_rate = 2e-5 / (1 - beta1**(int(i/400)+1))
            ...
            loss_new, g_new = self.pi_loss(x_new, self.growth_ratio)
...
# # Neo-Hooken
def energy_loss(pred, ws):
    ### Internal potential energy
    l1 = torch.sum(pred[0] * ws[0])
    ### Potential energy of the growth
    l2 = torch.sum(pred[1] * ws[0])
    ### Final loss
    loss = l1 + l2

    growth_ratio = pred[-1]

    return loss, l1, l2, growth_ratio
